/**
 * @file
 * Provides UI/UX progressive enhancements on TripalCultivateTheme's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.trpcultivate_theme.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-trpcultivate_theme-custom-color', changedInput.value);
    inputToSync.setAttribute('data-trpcultivate_theme-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-trpcultivate_theme-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-trpcultivate_theme-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes TripalCultivateTheme theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-trpcultivate_theme-custom-color',
      textInput.getAttribute('data-trpcultivate_theme-custom-color'),
    );
  ...
  /**
   * TripalCultivateTheme Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.trpcultivate_themeColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'trpcultivate_theme-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'trpcultivate_theme-color-picker',
        '[data-drupal-selector="trpcultivate_theme-color-picker"] input[type="text"]',
      );
